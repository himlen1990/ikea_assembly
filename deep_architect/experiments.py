...
import darch.my_evaluators as ev
...
def get_hyperparam_search_space(hps_type):
    ...
    if hps_type == "light":
        lr_inits = list( np.logspace(-2, -5, num=6) )
        rate_mults = [0.1, 0.5]
        rate_pats = [4, 8, 16, 32]
...
class CustomEvaluator:
    ...
    def __init__(self, train_dataset, val_dataset, in_d, cell_size,
    ...
    def eval_model(self, b):
        ...
        evaluator = ev.RegressionEvaluator(train_dataset=self.train_dataset,
...
#ikea-------------------------------------
def load_ikea_data(args):
    # information about the cifar-10
    in_d = (448, 448, 3)
    cell_size = 8
    # load ikea
    (Xtrain, ytrain, Xval, yval) = ds.load_ikea(
        data_dir='data/ikea/dataset')

    train_dataset = ds.InMemoryDataset(Xtrain, ytrain, True)
    val_dataset = ds.InMemoryDataset(Xval, yval, False)

    return (train_dataset, val_dataset, in_d, cell_size)
...
def get_search_space(args):
    ...
    num_classes = 10
    cell_size = 8
    num_output = cell_size * cell_size * 3
    ss = {'myconv' : srch_sp.my_convnet_ss0(num_output),
          'tfrefconv' : srch_sp.tfref_convnet_ss0(num_classes),
...
def run_searcher_with_checkpointing(args):
    ...
    #(train_dataset, val_dataset, test_dataset, in_d, nclasses) = load_ikea_data(args)
    (train_dataset, val_dataset, in_d, cell_size) = load_ikea_data(args)
...
def run_searcher_comparison_experiment(searcher_type, search_space_type, seed):
    args = {'augment_data' : True,
            ...
            'search_over_hyperparams_type' : 'light',
