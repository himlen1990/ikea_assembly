...
def load_ikea(data_dir):
    ...
    num_train = 280
    for name in glob.glob(os.path.join(data_dir, '*.jpg')):
        image_files.append(name)
        
    for image in image_files:
        img = cv2.imread(image)
        label_file = image[:-4] + '.npy'
    ...
    all_labels = np.array(label_processed)


    n = all_images.shape[0]
    inds = np.random.permutation(n)
    all_images = all_images[inds]
    all_labels = all_labels[inds]
    Xtrain = all_images[:num_train]
    Ytrain = all_labels[:num_train]

    Xval = all_images[num_train:]
    Yval = all_labels[num_train:]
